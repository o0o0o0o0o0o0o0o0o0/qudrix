...
const initializeWizard = () => {
  ...
  const wizardTabLinkElements = document.querySelectorAll(
    '.wizard-tab__nav-link'
  )
  const wizardDropdowns = document.querySelectorAll('.dropdown')
  const wizardAccessoriesTriggers = document.querySelectorAll(
    '.accessories-button'
  )
  const wizardAccessoriesSidebar = document.querySelector(
    '.wizard-sidebar__accessories-sidebar'
  )
  const wizardAccessoriesSidebarClose = document.querySelector(
    '.wizard-sidebar__accessories-sidebar-close'
  )
  const accessoriesElements = document.querySelectorAll(
    '.wizard-sidebar__elements-list.is--accessories .wizard-sidebar__element'
  )
  ...
  const colorElementsWrapper = document.querySelectorAll(
    '.color-elements__list'
  )
  const swivelPerogla = document.querySelector(
    '[data-name="Swivel Sliding pergola Q25"]'
  )
  const automaticSunscreen = document.querySelectorAll(
    '[data-accessory-name="Automatic sunscreen"]'
  )
  const swivelPeroglaColors = document.querySelector('[data-colors="roof"]')
  const automaticSunscreenColors = document.querySelector(
    '[data-colors="sunscreen"]'
  )
  ...
  const paidGlasses = document.querySelectorAll(
    '[data-name="Glass Window Paid"]'
  )
  ...
  wizardDropdowns.forEach((dropdown) => {
    dropdown.addEventListener('click', (e) => {
      ...
      const side = e.target.closest('[data-side]')
        ? e.target.closest('[data-side]').getAttribute('data-side')
        : null
  ...
  const checkIfHasActiveClass = (elem) => {
    if (
      ...
      elem.firstChild.classList.contains('active')
    ) {
      return true
    } else {
      return false
  ...
  const changeIcon = (element) => {
    const wizardElementIconDefault = element.querySelector(
      '.wizard-element__icon-default'
    )
    const wizardElementIconActive = element.querySelector(
      '.wizard-element__icon-active'
    )
  ...
  function handleRadioListClick(e) {
    ...
    if (
      ...
      (clickedElement.tagName === 'INPUT' &&
        ...
        !clickedElement.classList.contains('color-element__button') &&
        !clickedElement.parentElement.classList.contains(
          'color-element__button'
        ))
    ) {
      ...
      if (
        ...
        const accessories = clickedElement
          .closest('.wizard-sidebar__accessories-sidebar')
          .querySelectorAll('.wizard-sidebar__element.visible')
        accessories.forEach((accessory) => {
          if (
            accessory.getAttribute('data-accessory-name') !== 'No accessories'
          ) {
  ...
  const updateCodeElement = () => {
    codeElement.textContent = JSON.stringify(wizardParametrs, null, 2).replace(
      /,/g,
      ',\n'
    )
    ...
    summaryItems.forEach((item) => {
      ...
      if (dataItem === 'sides') {
        ...
        textElement.textContent = `Frame (${
          wizardParametrs[dataItem]['frame']
            ? wizardParametrs[dataItem]['frame']
            : 'None'
        }), Roof (${
          wizardParametrs[dataItem]['roof']
            ? wizardParametrs[dataItem]['roof']
            : 'None'
        }), Sunscreen (${
          wizardParametrs[dataItem]['sunscreen']
            ? wizardParametrs[dataItem]['sunscreen']
            : 'None'
        }),`
        ...
        textElement.textContent = wizardParametrs[dataItem]
          ? wizardParametrs[dataItem]['element-name']
          : 'None'
      }
    })
  }
  ...
  const wizardTabContent = document.querySelectorAll(
    '[data-tab], .wizard-sidebar__accessories-sidebar'
  )
    ...
    tab.addEventListener('click', (e) => {
      ...
      if (wizardParametrs['size']['element-name'] === 'Q2') {
        document.querySelector(
          '.tab-content__size-notification'
        ).style.display = 'block'
      } else {
        document.querySelector(
          '.tab-content__size-notification'
  ...
  function handleSideClick(tabText, sideText, target) {
    ...
    if (
      ...
      if (
        wizardParametrs[tabText][sideText]['accessory1-name'] &&
        wizardParametrs[tabText][sideText]['accessory2-name']
      ) {
  ...
  function handleAccessoriesClick(tabText, target) {
    const dataAccessory = target.parentElement.getAttribute(
      'data-accessory-name'
    )
    const dataSide = target.parentElement.getAttribute('data-side')
      ? target.parentElement.getAttribute('data-side')
      : null
    const dataTab = target.closest('[data-tab]').getAttribute('data-tab')
    // get index of clicked element with visible class
    const index = [
      ...target.parentElement.parentElement.querySelectorAll(
        '.wizard-sidebar__element.visible'
      ),
    ...
    if (
      ...
      if (!target.classList.contains('active')) {
        ...
        const accessories = target.parentElement.parentElement.querySelectorAll(
          '.wizard-sidebar__element.visible'
        )
        let activeAccessory = false
        accessories.forEach((accessory) => {
    ...
    if (
      ...
      if (!target.classList.contains('active')) {
        if (dataAccessory === 'No accessories') {
          ...
          wizardParametrs[tabText][dataSide][`accessory${index}-name`] =
            dataAccessory
        }
      } else {
        // remove current accessory from wizardParametrs
  ...
  function checkIfHasAccessoryTrigger(element) {
    const elementAccessoryTrigger = element.parentElement.querySelector(
      '.accessories-button'
    )
  ...
  function closeAccessoriesSidebar() {
    ...
    setTimeout(() => {
      wizardAccessoriesSidebar.style.display = 'none'
    }, 500)
  ...
  const handleAccessories = (element) => {
    const accessoriesDataName = element.parentElement.parentElement
      .querySelector('.wizard-sidebar__element-button')
      .getAttribute('data-name')
    const accessoriesDataSide = element.closest('[data-side]')
      ? element.closest('[data-side]').getAttribute('data-side')
      : null
    const accessoriesDataTab = element
      .closest('.wizard-tab__content-item')
      .getAttribute('data-tab')
    ...
    if (
      accessoriesDataSide !== 'null' &&
      accessoriesDataSide !== null &&
      accessoriesDataName !== 'null' &&
      accessoriesDataName !== null
    ) {
      ...
      accessoriesElements.forEach((element) => {
        ...
        if (
          elementDataSide === accessoriesDataSide &&
          element.getAttribute('data-accessory-to') === accessoriesDataName
        ) {
    ...
    let visibleAccessoriesElements = document.querySelectorAll(
      '.wizard-sidebar__elements-list.is--accessories .wizard-sidebar__element.visible'
    )
    ...
    visibleAccessoriesElements.forEach((element) => {
      ...
      if (accessoriesDataSide === null || accessoriesDataSide === 'null') {
        ...
        if (wizardParametrs[accessoriesDataTab]) {
          Object.values(wizardParametrs[accessoriesDataTab]).forEach(
            (value) => {
        ...
        if (wizardParametrs[accessoriesDataTab][accessoriesDataSide]) {
          Object.values(
            wizardParametrs[accessoriesDataTab][accessoriesDataSide]
          ).forEach((value) => {
            if (value === elementDataName) {
              // add class active to input
              element.firstElementChild.classList.add('active')
  ...
  function handleAccessoriesNoAccessories(element) {
    let accessoryLabel = element.querySelector(
      '.wizard-sidebar__element-button'
    )
  ...
  wizardAccessoriesTriggers.forEach((trigger) => {
    trigger.addEventListener('click', (e) => {
      ...
      accessoriesParametrs[
        e.target.closest('[data-tab]').getAttribute('data-tab')
      ] = {}
      checkIfAccessoriesHasActiveElement()
    })
  })
        ...
        element.addEventListener('click', (e) => {
          ...
          if (colorToggle.classList.contains('is--active')) {
            ...
            const otherInputs = document.querySelectorAll(
              `.color-tab__content-raw.is--default input:not([value="${input}"])`
            )
            ...
            const sameInputs = document.querySelectorAll(
              `.color-tab__content-raw.is--default input[value="${input}"]`
            )
  ...
  function handleFirstColorActive() {
    if (colorElementsWrapper) {
      colorElementsWrapper.forEach((wrapper) => {
        const visibilityParent = wrapper.closest(
          '.color-tab__content-raw.is--default'
        )

        if (visibilityParent) {
          const radioElements = wrapper.querySelectorAll(
    ...
    wrapper.addEventListener('click', (e) => {
      ...
      if (colorToggle.classList.contains('is--active')) {
        const colors = document.querySelectorAll(
          '.color-tab__content-raw.is--default .color-elements__list'
        )
        colors.forEach((color) => {
          const dataColor = color.getAttribute('data-color')
          const input = color.querySelector(
            '.color-radio__button.w--redirected-checked'
  ...
  wizardAccessoriesSidebarClose.addEventListener(
    'click',
    closeAccessoriesSidebar
  )
  let updCounter = 0
  updateCodeElement()
  ...
  function handleSession() {
    if (!sessionId) {
      ...
      initSession(wizardParametrs)
        .then((sessionData) => {
          const prodApp = 'https://model.qudrix.site/'
          const devApp = 'https://model.qudrix.site/'
